function main_run_llm_sfo_map45_only()
%MAIN_RUN_LLM_SFO_MAP45_ONLY Quick test: LLM-SFO on Map4 and Map5 only (after simplifying those maps + cfg).
%
% Outputs:
%   results/llm_sfo_map45_smoke.mat, results/llm_sfo_map45_smoke.csv
%   results/llm_weights_Map4.json, llm_weights_Map5.json
%   figures/llm_sfo_map45_smoke/
%   logs/llm_sfo_map45_smoke.log
...
fig_dir = fullfile(cfg.paths.figures, 'llm_sfo_map45_smoke');
ensure_dir(fig_dir);

log_file = fullfile(cfg.paths.logs, 'llm_sfo_map45_smoke.log');
fprintf(1, '=== LLM-SFO: Map4 + Map5 only | pop=%d iter=%d k=%d ===\n', ...
...
save(fullfile(cfg.paths.results, 'llm_sfo_map45_smoke.mat'), 'out');
writetable(out.records, fullfile(cfg.paths.results, 'llm_sfo_map45_smoke.csv'));
...
fprintf(1, 'See results/llm_sfo_map45_smoke.csv and figures/llm_sfo_map45_smoke/\n');
end
