function [bestSol, bestFit, curve, history] = run_sboa(problem, params)
%RUN_SBOA Secretary bird optimization algorithm (paper-inspired simplified).
...
for it = 1:T
    ...
    for i = 1:n
        if rand < 0.5
            % High-altitude scan (global search).
            candidate = X(i, :) + randn(1, dim) .* (ub - lb) * 0.12;
        else
            % Ground attack (local exploitation near best).
            candidate = X(i, :) + 0.7 * rand(1, dim) .* (bestSol - X(i, :)) + ...
                0.2 * rand(1, dim) .* (meanX - X(i, :));
        end
